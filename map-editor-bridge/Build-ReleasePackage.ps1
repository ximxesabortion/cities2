...
$packageBase = "MapEditorBridge-$version"
...
$buildArguments = @(
    "build",
    (Join-Path $codeRoot "MapEditorPlus.csproj"),
    "-c",
    "Release"
)
...
$installRoot = Join-Path $stageRoot "Mod\MapEditorPlus"
...
$runtimeHashLines = Get-ChildItem -LiteralPath $installRoot -File |
    ...
    ForEach-Object {
        ...
        "$($hash.Hash.ToLowerInvariant())  Mod/MapEditorPlus/$($_.Name)"
    }
...
Write-Host ""
Write-Host "Map Editor Bridge $version package completed."
